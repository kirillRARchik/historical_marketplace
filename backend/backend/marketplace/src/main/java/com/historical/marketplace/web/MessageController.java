...
public class MessageController {
    ...
    public ResponseEntity<?> createMessage(Authentication auth, @RequestBody CreateMessageRequest req) {
        ...
        if (receiver == null) return ResponseEntity.notFound().body(Map.of("error", "Receiver not found"));

        var product = productRepository.findById(req.productId()).orElse(null);
        if (product == null) return ResponseEntity.notFound().body(Map.of("error", "Product not found"));
